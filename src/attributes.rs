...
fn parse_value(value: &str) -> Option<UnitSystem> {
    ...
    // dioxus pixels are charicter based, so rem is the same as px
    else if let Ok(px) = value.trim_end_matches("rem").parse::<f32>() {
        Some(UnitSystem::Point(px))
    } else if let Ok(pct) = value.trim_end_matches('%').parse::<f32>() {
